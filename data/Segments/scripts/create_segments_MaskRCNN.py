...
from utilities.MaskRCNN import generate_mask
...
def main(dataset,create_segments=True, create_stats=True, device="cuda:0", check_existing=True):
    ...
    # load data
    prompt_specs = None
    train_paths, dataset_name = load_data(dataset, 'MaskRCNN',prompt_specs, check_existing)
    out_path = f"{dataset_name}_MaskRCNN"
    ...
    # save stats
    if os.path.exists(f"stats/{dataset}_MaskRCNN_STATS.torch"):
        old_stats = torch.load(f"stats/{dataset}_MaskRCNN_STATS.torch")
        stats.update(old_stats)   
    torch.save(stats, f"stats/{dataset}_MaskRCNN_STATS.torch")
    print(len(stats))

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Script to create segments using MaskRCNN")
